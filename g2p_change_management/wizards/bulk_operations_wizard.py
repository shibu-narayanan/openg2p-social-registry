...
class BulkOperationsWizard(models.TransientModel):
    ...
    reason = fields.Text(
        ...
        help="Provide a reason or comment for this bulk operation"
    )
    
    assign_to_user_id = fields.Many2one(
        'res.users',
        string="Assign To",
        help="Select user to assign the change requests to"
    )
    
    operation_summary = fields.Text(
    ...
    def action_confirm(self):
        ...
        try:
            ...
            message = _("Successfully %s %d change request(s)." % (operation.lower() + 'd', count))
            ...
            _logger.error("Bulk operation failed: %s", str(e))
            raise UserError(_("Bulk operation failed: %s") % str(e))
    ...
    def _validate_permissions(self):
        ...
        if self.operation_type in ['approve', 'reject']:
            return user.has_group('g2p_change_management.group_change_approver')
        elif self.operation_type in ['submit', 'cancel']:
            return user.has_group('g2p_change_management.group_change_user')
        elif self.operation_type == 'assign':
            return user.has_group('g2p_change_management.group_change_approver')
    ...
    def _bulk_approve(self):
        ...
        for change_request in self.change_request_ids:
            if change_request.state == 'submitted':
                change_request.with_context(
                    bulk_operation=True,
                    bulk_reason=self.reason
                ).action_approve()
            else:
                _logger.warning("Cannot approve change request %s: invalid state %s", 
                              change_request.name, change_request.state)
    ...
    def _bulk_reject(self):
        ...
        for change_request in self.change_request_ids:
            if change_request.state == 'submitted':
                ...
                _logger.warning("Cannot reject change request %s: invalid state %s", 
                              change_request.name, change_request.state)
    ...
    def _bulk_submit(self):
        ...
        for change_request in self.change_request_ids:
            if change_request.state == 'draft':
                ...
                _logger.warning("Cannot submit change request %s: invalid state %s", 
                              change_request.name, change_request.state)
    ...
    def _bulk_cancel(self):
        ...
        for change_request in self.change_request_ids:
            if change_request.state in ['draft', 'submitted']:
                ...
                _logger.warning("Cannot cancel change request %s: invalid state %s", 
                              change_request.name, change_request.state)
